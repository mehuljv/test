...
struct queue *g_req_queue_map[MAX_THREADS][MAX_QUEUES];
struct queue *g_rsp_queue_map[MAX_THREADS][MAX_QUEUES];
int g_num_slave_threads;
...
struct thread_info *create_thread_info(int id)
{
...
int add_req_q_to_thread_info(struct thread_info *thinfo, struct queue *q)
{
	if(!thinfo || !q)
		return -EINVAL;
...
int add_rsp_q_to_thread_info(struct thread_info *thinfo, struct queue *q)
{
	if(!thinfo || !q)
		return -EINVAL;
...
int setup_epoll_fd(struct list *list, int id)
{
	...
	while(node){
		...
		printf("Thread%d, Settingup Request Queue %p, efd %d\n", id, q, efd);
	}
	return efd;
}

int setup_master_epoll_fd(struct list *list, int id)
{
	struct list_node *node = list_get_first(list);	
	struct queue *q;
	struct epoll_event event;
...
struct queue *get_response_queue(int thread, int queue)
{
	return g_rsp_queue_map[thread][queue];
}
struct queue *get_request_queue(int x, int y, int z)
{
...
void *master_thread_func(void *data)
{
...
void *slave_thread_func(void *data)
{
	struct thread_info *thrinfo = (struct thread_info *)data;
	struct job_info *job;
...
int threadpool_test(int thread_count)
{
	int i = 0;
	...
	//Create one req_q & one rsp_q
	rsp_q = create_queue(FLAG_QUEUE_WITH_LOCK, 100, 0);
